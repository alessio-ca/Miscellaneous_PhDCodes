...
close all, clear all;
nSlice  = 204;                     % Pixels in z. Should be even because of midY=nSlice/2 and nFrames/2 (p 47, 123, 127). I changed to round(..)
nPixOut = 512;                  % Pixels in xy. Should be even since there is nPix/2. I changed to round(...)    
titl    = 'Series001';  % Name of input tif file
...
rFirst=.75;                  % Shouldn't be too small, cause try to make it at least 10 pixels. Smallest sphere radius. DB: Think this is in distance units, not pixel units!
maxjj=70;                   % Number of steps, i.e. maxjj * stepR = the biggest sphere that will be analyzed. Cause usually r is less than 6 mum.
stepR=0.05;                 % Following Jasnas Thesis (p 108), increment size is limited by the accuracy of this image analisys method, 1/3 of a pixel. 
...
maxnumPixels        = 30000;                    % Get rid of it. Don't change
    FD1b  % Enter=accept current sphere; 1+enter=reject current sphere; 2+enter=reject all that follow; 4+enter=enter number of spheres to accept in a row. 
