function pos_list=Ftrack_Fin (filehead,first,last,feature)
% 
% USEAGE:   pos_list=Ftrack_Fin (filehead,first,last,feature);
% PURPOSE:  
...
%  Modifications: Alessio Caciagli, University of Cambridge, December 2015
...
for frame=first:last
    ...
    % read in file
    %image = double(imcomplement(imread([filehead, num2str(frame,'%06u'),'.tiff']))); %Appropriate for 6 trailing zeroes in filename. MODIFY HERE if you have different number of zeroes in filename!!!
    image = double(imread([filehead, num2str(frame,'%06u'),'.tiff'])); %Appropriate for 6 trailing zeroes in filename. MODIFY HERE if you have different number of zeroes in filename!!!
    ...
    % Find locations of the brightest pixels.
    % Need to change the 'th' (threshold) argument to something that is
    % specified or determined. Current value is 0.6. 
    % A rough guide is to accept 60-70% of the brightest pixel. 
    pk = pkfnd(imagebp, 0.5*max(imagebp(:)), feature);
...
% Format the position list so that we have four columns: frame, x, y, m_0, m_2
pos_list = pos_list';
