...
for frame=1:listlen
    ...
    omega=fft_acf(:,1);
    Gpx = -rheofactor(1).*fft_acf(:,4)./(2*pi*fft_acf(:,1).*fft_acf(:,6)); %Uncorrected for trap for visualization purposes
    Gpy = -rheofactor(2).*fft_acf(:,5)./(2*pi*fft_acf(:,1).*fft_acf(:,7)); %Uncorrected for trap for visualization purposes
    Gppx = -rheofactor(1).*fft_acf(:,2)./(2*pi*fft_acf(:,1).*fft_acf(:,6)); 
    Gppy = -rheofactor(2).*fft_acf(:,3)./(2*pi*fft_acf(:,1).*fft_acf(:,7));
    clear fft_acf
    %%
    ax3=subplot(2,2,2);
    loglog(omega,Gpx,'*')
