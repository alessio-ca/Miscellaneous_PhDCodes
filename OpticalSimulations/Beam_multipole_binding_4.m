...
%% 4 INTERACTING DIPOLES: ITERATIVE SOLVER
...
parfor k = 1:targetWorkCount
    r4=Point(rx(k),ry(k),rz(k));
    phase_r4 = focus_phase(bg,f,r4.X,r4.Y,r4.Z,'er',nm^2);
    ...
    if (norm(Point(rx(k),ry(k),rz(k)) - r1)<(2*a - 6*meshStep) ||...
        norm(Point(rx(k),ry(k),rz(k)) - r2)<(2*a - 6*meshStep) ||...
        norm(Point(rx(k),ry(k),rz(k)) - r3)<(2*a - 6*meshStep)   ...
        ) 
        Ex(k)=0;
        Ey(k)=0;
        Ez(k)=0;
        ...
        Bi_n_base=[focus_fast_B(E_inc,E_focus_1);focus_fast_B(E_inc,E_focus_2);focus_fast_B(E_inc,E_focus_3);focus_fast_B(E_inc,E_focus_4)];
...
%         end
        ...
        for i=1:length(rP)-1
                eTemp = focus_fast_E_dip(idP(i),r4,Ei_n(i,:));
                bTemp = focus_fast_B(E_inc,eTemp);
                Ex(k) =  Ex(k)+eTemp(1,1);
                Ey(k) =  Ey(k)+eTemp(1,2);
                Ez(k) =  Ez(k)+eTemp(1,3);
        ...
        [x_temp,y_temp,z_temp] = meshgrid(r4.X-2*meshStep:meshStep:r4.X+2*meshStep,r4.Y-2*meshStep:meshStep:r4.Y+2*meshStep,0);
        ...
        [F_temp,~,~] = idP(numDip).force_general(r4,E_temp,B_temp,2);
...
save('Force_4_beam_reduced.mat','F')

%% Optional: load the calculated fields
load('Force_4_beam_reduced.mat')
...
%% Optional: export of the calculated fields
save('ForceGx_4_beam_reduced.mat','FxG')
save('ForceGy_4_beam_reduced.mat','FyG')
save('divF_4_beam_reduced.mat','divF')
