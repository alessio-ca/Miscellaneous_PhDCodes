...
Nphi = 16;
Nr = 10;
bg = BeamGauss(Ex0,Ey0,w0,R,Nphi,Nr,'lambda0',1064e-9,'er',nm^2);
bg = bg.normalize(60*64*2/10000);
E_inc = EFieldFocus(bg,f,'lambda0',1064e-9,'er',nm^2);
...
%% 8 INTERACTING DIPOLES: ITERATIVE SOLVER
...
idP_F = InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r1);
[ur_r1,uth_r1,r_r1]=focus_fast_E_Vector_Sph(r1);
...
z2=0;
r2=Point(x2,y2,z2);
phase_r2 = focus_phase(bg,f,x2,y2,z2,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r2)];
[ur_r2,uth_r2,r_r2]=focus_fast_E_Vector_Sph(r2);
...
z3=0;
r3=Point(x3,y3,z3);
phase_r3 = focus_phase(bg,f,x3,y3,z3,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r3)];
[ur_r3,uth_r3,r_r3]=focus_fast_E_Vector_Sph(r3);
...
z4=0;
r4=Point(x4,y4,z4);
phase_r4 = focus_phase(bg,f,x4,y4,z4,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r4)];
[ur_r4,uth_r4,r_r4]=focus_fast_E_Vector_Sph(r4);
...
z5=0;
r5=Point(x5,y5,z5);
phase_r5 = focus_phase(bg,f,x5,y5,z5,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r5)];
[ur_r5,uth_r5,r_r5]=focus_fast_E_Vector_Sph(r5);
...
z6=0;
r6=Point(x6,y6,z6);
phase_r6 = focus_phase(bg,f,x6,y6,z6,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r6)];
[ur_r6,uth_r6,r_r6]=focus_fast_E_Vector_Sph(r6);
...
z7=0;
r7=Point(x7,y7,z7);
phase_r7 = focus_phase(bg,f,x7,y7,z7,'er',nm^2);
idP_F = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r7)];
[ur_r7,uth_r7,r_r7]=focus_fast_E_Vector_Sph(r7);
...
parfor k = 1:targetWorkCount
    ...
    r8=Point(rx(k),ry(k),rz(k));
    phase_r8 = focus_phase(bg,f,r8.X,r8.Y,r8.Z,'er',nm^2);
    idP = [idP_F,InducedDipole(alpharc,'lambda0',lambda0,'er',nm^2,'rd',r8)];
    [ur_r8,uth_r8,r_r8]=focus_fast_E_Vector_Sph(r8);


    rP = [r1,r2,r3,r4,r5,r6,r7,r8];
    ur = [ur_r1;ur_r2;ur_r3;ur_r4;ur_r5;ur_r6;ur_r7;ur_r8];
    uth = [uth_r1;uth_r2;uth_r3;uth_r4;uth_r5;uth_r6;uth_r7;uth_r8];
    r_V = [r_r1;r_r2;r_r3;r_r4;r_r5;r_r6;r_r7;r_r8];   
    
    chknorm = zeros(1,numDip-1);
    for ii = 1:numDip-1
        chknorm(ii) = norm(Point(rx(k),ry(k),rz(k)) - rP(ii))<(2*a - 6*meshStep);
    end
    ...
    if (any(chknorm)) 
        ...
        E_focus_3 = focus_fast_E(bg,f,phase_r3,'er',nm.^2);
        E_focus_4 = focus_fast_E(bg,f,phase_r4,'er',nm.^2);
        E_focus_5 = focus_fast_E(bg,f,phase_r5,'er',nm.^2);
        E_focus_6 = focus_fast_E(bg,f,phase_r6,'er',nm.^2);
        E_focus_7 = focus_fast_E(bg,f,phase_r7,'er',nm.^2);
        E_focus_8 = focus_fast_E(bg,f,phase_r8,'er',nm.^2);


        Ei_n_base = [E_focus_1(1,:);E_focus_2(1,:);E_focus_3(1,:);E_focus_4(1,:);...
                     E_focus_5(1,:);E_focus_6(1,:);E_focus_7(1,:);E_focus_8(1,:)...
                    ];
        Bi_n_base=[focus_fast_B(E_inc,E_focus_1);focus_fast_B(E_inc,E_focus_2);...
                   focus_fast_B(E_inc,E_focus_3);focus_fast_B(E_inc,E_focus_4);...
                   focus_fast_B(E_inc,E_focus_5);focus_fast_B(E_inc,E_focus_6);...
                   focus_fast_B(E_inc,E_focus_7);focus_fast_B(E_inc,E_focus_8)...
                   ];
        ...
        while tol > 0.001 && cc < 1000
            ...
            for i=1:numDip
                for j=1:numDip
                    if i~=j
%                       eTemp = focus_fast_E_dip(idP(j),rP(i),Ei_o(j,:),ur(i,:),uth(i,:),r_V(i,:));
...
%         end
        %Field calculation
        Ex(k)=Ei_n_base(numDip,1);
        Ey(k)=Ei_n_base(numDip,2);
        Ez(k)=Ei_n_base(numDip,3);
        Bx(k)=Bi_n_base(numDip,1);
        By(k)=Bi_n_base(numDip,2);
        Bz(k)=Bi_n_base(numDip,3);
        
        for i=1:numDip-1
                eTemp = focus_fast_E_dip(idP(i),rP(numDip),Ei_n(i,:));
                bTemp = focus_fast_B(E_inc,eTemp);
                Ex(k) =  Ex(k)+eTemp(1,1);
                Ey(k) =  Ey(k)+eTemp(1,2);
                Ez(k) =  Ez(k)+eTemp(1,3);
        ...
        [x_temp,y_temp,z_temp] = meshgrid(rP(numDip).X-2*meshStep:meshStep:rP(numDip).X+2*meshStep,rP(numDip).Y-2*meshStep:meshStep:rP(numDip).Y+2*meshStep,0);
        ...
        [F_temp,~,~] = idP(numDip).force_general(rP(numDip),E_temp,B_temp,2);
...
save('Force_8_beam_reduced.mat','F')

%% Optional: load the calculated fields
load('Force_8_beam_reduced.mat')

%% Spline interpolant

F.Vx((F.X-x1).^2 + (F.Y-y1).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x2).^2 + (F.Y-y2).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x3).^2 + (F.Y-y3).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x4).^2 + (F.Y-y4).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x5).^2 + (F.Y-y5).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x6).^2 + (F.Y-y6).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x7).^2 + (F.Y-y7).^2 < (2*a-3*meshStep)^2 ...
     )=NaN;
F.Vy((F.X-x1).^2 + (F.Y-y1).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x2).^2 + (F.Y-y2).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x3).^2 + (F.Y-y3).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x4).^2 + (F.Y-y4).^2 < (2*a-3*meshStep)^2 |...
     (F.X-x5).^2 + (F.Y-y5).^2 < (2*a-3*meshStep)^2 |...
...
%% Optional: export of the calculated fields
save('ForceGx_8_beam_reduced.mat','FxG')
save('ForceGy_8_beam_reduced.mat','FyG')
save('divF_8_beam_reduced.mat','divF')
...
points=points((xprobe-x1).^2 + (yprobe-y1).^2 > (2*a)^2 & ...
              (xprobe-x2).^2 + (yprobe-y2).^2 > (2*a)^2 & ...
              (xprobe-x3).^2 + (yprobe-y3).^2 > (2*a)^2 ...
              ,:);
...
parfor i = 1:length(points)
    ...
    [x_out,traj] = graddesc(x_in,fun_x,fun_y,[x1,y1;x2,y2;x3,y3],2*a,[Lx,Ly],[-Lx,-Ly],'PointOut',1);
    points_out(i,:) = x_out;
    traj_x(:,i)=traj(:,1);
    traj_y(:,i)=traj(:,2);
end
...
points_out=points_out((points_out(:,1)-x2).^2 + (points_out(:,2)-y2).^2 > 4*a^2,:);
points_out=points_out((points_out(:,1)-x3).^2 + (points_out(:,2)-y3).^2 > 4*a^2,:);
points_out=points_out((points_out(:,1)-x4).^2 + (points_out(:,2)-y4).^2 > 4*a^2,:);
points_out=points_out((points_out(:,1)-x5).^2 + (points_out(:,2)-y5).^2 > 4*a^2,:);
points_out=points_out((points_out(:,1)-x6).^2 + (points_out(:,2)-y6).^2 > 4*a^2,:);
points_out=points_out((points_out(:,1)-x7).^2 + (points_out(:,2)-y7).^2 > 4*a^2,:);

points_out=points_out(points_out(:,3)==1,:);
