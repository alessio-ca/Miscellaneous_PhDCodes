...
%              It's a Ny*Nl matrix. 
%   - Nbg: specify the constant background condition (1=active, 0=inactive).
...
if  UserL  == 1
    if Nbg == 1 %Constant background condition
...
if Ny > Nx
    [C,eta]=CRIE_qr(diag(1./sqrt(wt))*A,y./sqrt(wt));
else
    C = A;
    eta = y;
end
...
if Ny0 == 1
    ...
    if Nbg == 1
        Aeq = [Aeq,1,ones(1,Nl)]; %y(0) = 1 constraint + bg terms
    else
