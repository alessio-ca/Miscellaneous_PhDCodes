...
%% Computation
Lx_LDP = zeros(size(alphaV));
sol_LDP = zeros(size(alphaV));
lambdaV_LDP = zeros(size(alphaV));
...
%for i = 1:1
    ...
    %Solve the constrained linear LS problem via LDP to NNLS conversion
    GlsA = Gls*Stilde;
    hlsA = hls - Anq(:,1:end-1)*K2*Z*H1*W*Stilde*gammatilde; 
    Els = [GlsA';hlsA'];
    fls = [zeros(Nxye,1);1];
    [epsilon,~,~,exitflag] = lsqnonneg(Els,fls,options);
    epsilon = Els*epsilon - fls;
    epsilon = -epsilon./epsilon(end);
    epsilon(end)=[];
    s_LDP = K1*s1 + K2*Z*H1*(W*Stilde*(epsilon + gammatilde) + r1);
    
    sol_LDP(i) = norm(A*s_LDP - y);
    Lx_LDP(i) = norm(R*s_LDP);
    lambdaV_LDP(i) = alphaP;
    if exitflag == 0
        sol_LDP(i) = [];
        Lx_LDP(i) = [];
        lambdaV_LDP(i) = [];
...
sol_LDP = nonzeros(sol_LDP);
Lx_LDP = nonzeros(Lx_LDP);
lambdaV_LDP = nonzeros(lambdaV_LDP);
%%
% Corner of L_curve and recalculation with optimal alpha
[k_c,info] =  CONTIN_Alessio_corner(sol_LDP,Lx_LDP);
lambda_LDP = lambdaV_LDP(k_c);

gammatilde = gamma(1:Nxye).*Sdiag./sqrt(Sdiag.^2 + lambda_LDP.^2);
Stilde = diag(sqrt(Sdiag.^2 + lambda_LDP^2));
Stilde = diag(1./diag(Stilde)); %Inverse
...
plot(g,s_LDP)
figure(2)
semilogx(t,y,t,yfit_LDP,'*')
figure(3)
scatter(sol_LDP,Lx_LDP)
toc
