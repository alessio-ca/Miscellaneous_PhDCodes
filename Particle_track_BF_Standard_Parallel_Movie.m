...
[poslist,histostep]=AnalyseTrack_Parallel_Movie(video,filter,ppm,progressStep,r_max*2,10);
figure(1)
histogram(histostep);
[video,offset,nfr]=movie2frame(filename,offset);
while length(video)>1
    [poslist,histostep]=AnalyseTrack_Parallel_Movie(video,filter,ppm,progressStep,r_max*2,10,poslist); 
    [video,offset,nfr]=movie2frame(filename,offset);
end
ppm.delete()
