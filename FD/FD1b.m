...
while(i >= 1)              % for all spheres found
        ...
        for k=i:-1:(i-tmp1)         
            ...
            for m = -deltaZ:deltaZ
                if zd + m > 0 && zd + m < nFrames
                   deltaPix = double(uint16(sqrt(double(uint16(grandresult(k,4)/ddx+.5)-1)^2 - double(uint16(m^2*(ddx/ddz)+.5)-1)^2)));
                   for n = -deltaPix:deltaPix 
    ...
    elseif tmp == 2  % Rejects this and all the following spheres if tmp =2.
        ...
        for m = -deltaZ:deltaZ
            if zd + m > 0 && zd + m < nFrames
               deltaPix = double(uint16(sqrt(double(uint16(grandresult(i,4)/ddx+.5)-1)^2 - double(uint16(m^2*(ddx/ddz)+.5)-1)^2)));
                   for n = -deltaPix:deltaPix 
                      if x+n > 0 && x+n < nPix,  A(x+n, y, zd+m) = 255; end    % ? cross
                      if y+n > 0 && y+n < nPix,  A(x, y+n, zd+m) = 255; end    % ? cross
                   end
