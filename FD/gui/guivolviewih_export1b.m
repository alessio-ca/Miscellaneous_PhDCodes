...
% Removed downsizing from 512x512 to 256x256; not sure why this was ever advantageous. Now hardcoded for 512x512. (DB 21/11/13)

%Callbacks where all the action happends:
%figure1_WindowButtonUpFcn
function varargout = guivolviewih_export(varargin)
% GUIVOLVIEWIH_EXPORT1B M-file for guivolviewih_export1b.fig
%      GUIVOLVIEWIH_EXPORT1B, by itself, creates a new GUIVOLVIEWIH_EXPORT1B or raises the existing
%      singleton*.
%
%      H = GUIVOLVIEWIH_EXPORT1B returns the handle to a new GUIVOLVIEWIH_EXPORT1B or the handle to
%      the existing singleton*.
%
%      GUIVOLVIEWIH_EXPORT1B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIVOLVIEWIH_EXPORT1B.M with the given input arguments.
%
%      GUIVOLVIEWIH_EXPORT1B('Property','Value',...) creates a new GUIVOLVIEWIH_EXPORT1B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guivolviewih_export1b_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guivolviewih_export1b_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guivolviewih_export1b
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @guivolviewih_export1b_OpeningFcn, ...
                   'gui_OutputFcn',  @guivolviewih_export1b_OutputFcn, ...
                   'gui_LayoutFcn',  @guivolviewih_export1b_LayoutFcn, ...
                   'gui_Callback',   []);
...
% --- Executes just before guivolviewih_export1b is made visible.
function guivolviewih_export1b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guivolviewih_export1b (see VARARGIN)

% Choose default command line output for guivolviewih_export1b
...
zspp = input('Please enter the aspect ratio (nSnell*(boxZ/PixZ)/(boxX/PixX)) \n');
...
% so window can get raised using guivolviewih_export1b.
% global output
...
% UIWAIT makes guivolviewih_export1b wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guivolviewih_export1b_OutputFcn(hObject, eventdata, handles)
...
% Get an aspect ratio 1.33 for older data, 2.16 for newer data
    answer=inputdlg('Input zsp:','Aspect ratio for image file', 1, {num2str(zspp)});
    if ~isempty(answer)
...
if ~isequal(file, 0)
    ...
    if ext=='.mat' %This loads up *.mat file data
        ...
        %For some reason the radius is in microns, whilst everything else is in
        %pixels....So convert to pixels:
        spherelist.sr(start:start+len,1)=droplets(:,4)/(boxX/512.0); % x or y divided by 256; changed to 512 (DB: 21/11/13)
...
if ~isequal(file,0)    
    ...
    droplets(:,4)=spherelist.sr*(boxX/512.0);
...
if on
    if spherelist.sr(1)~=0
        ...
        overlayed=cirpaint2b(spherelist.sy,spherelist.sx,spherelist.sz,spherelist.sr,output(:,:,nslice),nslice,zsp,mode);
    else
...
function h1 = guivolviewih_export1b_LayoutFcn(policy)
...
'Position',[200 500 800 562],...
'Renderer',get(0,'defaultfigureRenderer'),...
'RendererMode','manual',...
'Resize','off',...
'WindowButtonMotionFcn','guivolviewih_export1b(''figure1_WindowButtonMotionFcn'',gcbo,[],guidata(gcbo))',...
'WindowButtonUpFcn','guivolviewih_export1b(''figure1_WindowButtonUpFcn'',gcbo,[],guidata(gcbo))',...
'ButtonDownFcn','guivolviewih_export1b(''figure1_ButtonDownFcn'',gcbo,[],guidata(gcbo))',...
...
'ZTickMode','manual',...
'ButtonDownFcn','guivolviewih_export1b(''axes1_ButtonDownFcn'',gcbo,[],guidata(gcbo))',...
'CreateFcn','guivolviewih_export1b(''axes1_CreateFcn'',gcbo,[],guidata(gcbo))',...
'Tag','axes1',...
'UserData',zeros(1,0));
...
h7 = uimenu(...
'Parent',h1,...
'Callback','guivolviewih_export1b(''FileMenu_Callback'',gcbo,[],guidata(gcbo))',...
'Label','File',...
'Tag','FileMenu');

h8 = uimenu(...
'Parent',h7,...
'Callback','guivolviewih_export1b(''OpenMenuItem_Callback'',gcbo,[],guidata(gcbo))',...
'Label','Open Image...',...
'Tag','OpenMenuItem');

h9 = uimenu(...
'Parent',h7,...
'Callback','guivolviewih_export1b(''OpenDropletMenuItem_callback'',gcbo,[],guidata(gcbo))',...
'Label','Open Droplet file...',...
'Tag','OpenDropletMenuItem');

h10 = uimenu(...
'Parent',h7,...
'Callback','guivolviewih_export1b(''SaveDropletMenuItem_Callback'',gcbo,[],guidata(gcbo))',...
'Label','Save Droplet file...',...
'Tag','SaveDropletMenuItem');

h11 = uimenu(...
'Parent',h7,...
'Callback','guivolviewih_export1b(''CloseMenuItem_Callback'',gcbo,[],guidata(gcbo))',...
'Label','Close',...
'Separator','on',...
'Tag','CloseMenuItem');
...
'Callback','guivolviewih_export1b(''slider2_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Max',59,...
'Min',1,...
'Position',[537 35 20 512],...
'String',{ '' },...
'Style','slider',...
'SliderStep',[0.017 0.085],...
'Value',20,...
'ButtonDownFcn','guivolviewih_export1b(''slider2_ButtonDownFcn'',gcbo,[],guidata(gcbo))',...
'CreateFcn','guivolviewih_export1b(''slider2_CreateFcn'',gcbo,[],guidata(gcbo))',...
'Tag','slider2');
...
'ListboxTop',0,...
'Position',[95 37 15.8 1.38461538461538],...
'String','Slice:',...
'Style','text',...
'Tag','slicenumber');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 35.4 21.4 1.15384615384615],...
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 33.8 15.8 1.46153846153846],...
'String','x:',...
'Style','text',...
'Tag','cursorx');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 32.2 15.8 1.61538461538462],...
'String','y:',...
'Style','text',...
'Tag','cursory');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 30.6 15.8 1.69230769230769],...
'String','z:',...
'Style','text',...
'Tag','cursorz');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 28 19.2 1.23076923076923],...
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 26.4 21.2 1.46153846153846],...
'String','x:',...
'Style','text',...
'Tag','spx');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 24.8 21.4 1.46153846153846],...
'String','y:',...
'Style','text',...
'Tag','spy');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 23.2 20.6 1.69230769230769],...
'String','z:',...
'Style','text',...
'Tag','spz');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 21.6 21.4 1.46153846153846],...
'String','Index:',...
'Style','text',...
'Tag','spi');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 20 20.4 1.53846153846154],...
'String','r:',...
'Style','text',...
'Tag','spr');
...
'HorizontalAlignment','left',...
'ListboxTop',0,...
'Position',[95 18.4 20.2 1.15384615384615],...
'String','Intensity:',...
'Style','text',...
'Tag','spint');
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''clear_pointlist_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[95 7 15.8 1.76923076923077],...
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''fit_sphere_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[95 4.61538461538462 16 1.76923076923077],...
'String','Fit sphere',...
'Tag','fit_sphere');
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''delete_sphere_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[95 2.23076923076923 16 1.76923076923077],...
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''clear_spherelist_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[112.4 7 15.8 1.76923076923077],...
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''tidy_spherelist_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[112.4 4.61538461538462 15.8 1.76923076923077],...
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''find_neighbours_Callback'',gcbo,[],guidata(gcbo))',...
'ListboxTop',0,...
'Position',[112.4 2.23076923076923 15.8 1.76923076923077],...
...
'Units','characters',...
'ListboxTop',0,...
'Position',[95 16 17.2 1.38461538461538],...
'String','Overlay Mode:',...
'Style','text',...
'Tag','text18');
...
'Parent',h1,...
'Units','characters',...
'Callback','guivolviewih_export1b(''overlay_mode_Callback'',gcbo,[],guidata(gcbo))',...
'Max',0,...
'Min',1,...
'Position',[95 10 18 5.46153846153846],...
'String',{ 'White outline' 'White filled' 'Black outline' 'Black filled' 'White crosses' 'Black crosses' 'None' },...
'Style','listbox',...
'Value',1,...
'CreateFcn','guivolviewih_export1b(''overlay_mode_CreateFcn'',gcbo,[],guidata(gcbo))',...
'Tag','overlay_mode');
...
if numargin == 0
    % GUIVOLVIEWIH_EXPORT1B
    % create the GUI
    gui_Create = 1;
elseif numargin > 3 & ischar(varargin{1}) & ishandle(varargin{2})
    % GUIVOLVIEWIH_EXPORT1B('CALLBACK',hObject,eventData,handles,...)
    gui_Create = 0;
else
    % GUIVOLVIEWIH_EXPORT1B(...)
